...
public class IAFinanzasService : IIAFinanzasService
{
    ...
    {
        // Priorizamos "supermercado" para evitar que acabe en categor√≠as amplias
    ...
    public async Task<EntrenamientoModeloCategoriasResponseDto> EntrenarModeloCategoriasAsync(bool forzar = false, CancellationToken cancellationToken = default)
    {
        ...
        if (!File.Exists(datasetPath))
            throw new BadRequestException($"No se ha encontrado el dataset de categor√≠as en '{datasetPath}'.");
        ...
        try
        {
            ...
            if (categorias.Length < 2)
                throw new BadRequestException("Se requieren al menos dos categor√≠as distintas para entrenar el modelo.");
    ...
    public async Task<SugerenciaCategoriaResponseDto> SugerirCategoriaAsync(SugerenciaCategoriaRequestDto request, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.Descripcion))
            throw new BadRequestException("La descripci√≥n es obligatoria para sugerir categor√≠a.");
        ...
        if (categoriasCandidatas.Count == 0)
            throw new BadRequestException("No hay categor√≠as disponibles para sugerir con el tipo indicado.");
        ...
        {
            if (_modelo == null)
                throw new InvalidOperationException("Ahora mismo no podemos recomendar una categorÌa.");
